...
const FormCompany = ({ site_location }) => {
  ...
  useEffect(() => {
    axios
      .get(`${site_location}:3001/api/policyMasterList/dataById/${params.id}`)
      .then((response) => {
        setTypeId(response.data[0].typeId)
        setNameType(response.data[0].nameType)
  ...
  const saveData = (e) => {
    ...
    axios
      .post(`${site_location}:3001/api/policyMasterList/updateData`, {
        id: params.id,
        name: name,
        details: details,
      })
      .then((response) => {
        ...
        setTimeout(() => {
          ...
          navigate(`/policyMaster/policy/${typeId}`, { replace: true })
        }, 2000)
      })
  }
    ...
    <>
      <CCard className="mb-4">
        <CCardHeader>
          ...
          <strong className="text-primary">{nameType}</strong> / แก้ไขรายการนโยบายความเป็นส่วนตัว
        </CCardHeader>
        <CCardBody>
