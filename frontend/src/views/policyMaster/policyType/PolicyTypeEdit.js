...
const FormCompany = ({ site_location }) => {
  ...
  useEffect(() => {
    axios
      .get(`${site_location}:3001/api/policyMasterType/dataById/${params.id}`)
      .then((response) => {
        setName(response.data[0].name)
        setTitle(response.data[0].title)
        setPar1(response.data[0].par1)
        setPar2(response.data[0].par2)
        setFormType(response.data[0].formType)
      })
      .catch((error) => {
        console.log(error)
      })
  ...
  const saveData = (e) => {
    e.preventDefault()
    axios
      .post(`${site_location}:3001/api/policyMasterType/updateData`, {
        name: name,
        title: title,
        par1: par1,
        par2: par2,
