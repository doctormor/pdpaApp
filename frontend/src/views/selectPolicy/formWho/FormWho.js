...
const FormCookies = ({ site_location }) => {
  ...
  useEffect(() => {
    ...
    axios.get(`${site_location}:3001/api/formList/dataByUserId/${userId}/5`).then((response) => {
      axios
        .get(`${site_location}:3001/api/policyMasterList/5`)
        .then((responses) => {
  ...
  return (
    <>
      <CCard className="mb-4">
        <CCardHeader>
          <CIcon icon={cilBrowser} className="me-2" /> แบบฟอร์มบันทึกการเปิดเผยข้อมูล
        </CCardHeader>
        <CCardBody>
          <p className="h5">คุณเปิดเผยข้อมูลกับใครบ้าง</p>
          <hr />
