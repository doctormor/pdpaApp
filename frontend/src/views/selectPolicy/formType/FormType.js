...
const FormCookies = ({ site_location }) => {
  ...
  useEffect(() => {
    ...
    axios.get(`${site_location}:3001/api/formList/dataByUserId/${userId}/2`).then((response) => {
      axios
        .get(`${site_location}:3001/api/policyMasterList/2`)
        .then((responses) => {
  ...
  return (
    <>
      <CCard className="mb-4">
        <CCardHeader>
          <CIcon icon={cilBrowser} className="me-2" /> แบบฟอร์มบันทึกประเภทข้อมูลที่เก็บ
        </CCardHeader>
        <CCardBody>
          <p className="h5">รายละเอียดประเภทข้อมูลที่เก็บ</p>
          <hr />
