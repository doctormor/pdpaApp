...
const FormCookies = ({ site_location }) => {
  ...
  useEffect(() => {
    ...
    axios.get(`${site_location}:3001/api/formData/dataByUserId/${userId}`).then((response) => {
      if (response.data != '') {
        ...
        setDetails(response.data[0].dayStorage)
      }
    })
  ...
  const saveData = (e) => {
    ...
    // Update Data
    axios
      .post(`${site_location}:3001/api/formData/updateDayStorage`, {
        formId: formId,
        details: details,
      })
      .then((response) => {
  ...
  return (
    <>
      <CCard className="mb-4">
        <CCardHeader>
          <CIcon icon={cilBrowser} className="me-2" /> แบบฟอร์มบันทึกข้อมูลระยะเวลาในการจัดเก็บ
        </CCardHeader>
        <CCardBody>
          <p className="h5">รายละเอียดข้อมูลระยะเวลาในการจัดเก็บ</p>
          <hr />
