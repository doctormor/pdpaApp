...
const FormCookies = ({ site_location }) => {
  ...
  useEffect(() => {
    ...
    axios.get(`${site_location}:3001/api/formList/dataByUserId/${userId}/3`).then((response) => {
      axios
        .get(`${site_location}:3001/api/policyMasterList/3`)
        .then((responses) => {
  ...
  return (
    <>
      <CCard className="mb-4">
        <CCardHeader>
          <CIcon icon={cilBrowser} className="me-2" /> แบบฟอร์มบันทึกสถานที่เก็บข้อมูล
        </CCardHeader>
        <CCardBody>
          <p className="h5">รายละเอียดสถานที่เก็บข้อมูล</p>
          <hr />
